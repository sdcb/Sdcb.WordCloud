...
internal record struct OrientationRect(HorizontalOrVertical Orientations, SKRectI Rect, SKPointI Center)
{
    ...
    internal static OrientationRect ExpandHorizontally(SKPointI center, SKSizeI size)
    {
        ...
        return new (HorizontalOrVertical.Horizontal, new SKRectI(center.X - halfWidth, center.Y - halfHeight, center.X + otherHalfWidth, center.Y + otherHalfHeight), center);
    }
    ...
    internal static OrientationRect ExpandVertically(SKPointI center, SKSizeI size)
    {
        ...
        return new(HorizontalOrVertical.Vertical, new SKRectI(center.X - halfHeight, center.Y - halfWidth, center.X + otherHalfHeight, center.Y + otherHalfWidth), center);
    }
...
#pragma warning disable CS8524 // switch 表达式不会处理其输入类型的某些值(它不是穷举)，这包括未命名的枚举值。
        ...
        {
            HorizontalOrVertical.Horizontal => 0,
            HorizontalOrVertical.Vertical => 90,
        };
    }
}
